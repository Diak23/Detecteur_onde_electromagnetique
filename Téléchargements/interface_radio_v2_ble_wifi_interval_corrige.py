...
INTERFACE_WIFI_MONITOR = "wlan0mon"
...
ble_thread = None
ble_last_message = None
ble_last_rssi = None
ble_last_mac = None
ble_last_intervalle = None
ble_last_seq = 0
ble_consumed_seq = 0
dernier_temps_ble = {}

wifi_beacon_running = False
wifi_beacon_thread = None
wifi_last_message = None
wifi_last_rssi = None
wifi_last_mac = None
wifi_last_intervalle = None
wifi_last_seq = 0
wifi_consumed_seq = 0
dernier_temps_wifi = {}
...
def creer_socket_udp():
    ...
    try:
        ...
        sock = None
        label_etat.config(text=f"Erreur UDP port {UDP_PORT} : {e}")
        return False


def fermer_socket_udp():
    global sock
...
# =========================
# BEACONS WIFI AVEC INTERVALLE
# =========================

def scanner_wifi_beacons_thread():
    global wifi_beacon_running
    global wifi_last_message, wifi_last_rssi, wifi_last_mac, wifi_last_intervalle
    global wifi_last_seq
    ...
    try:
        ...
        while wifi_beacon_running:
            ...
            ssid = champs[0] if champs[0] != "" else "SSID caché"
            mac = champs[1]
            rssi_txt = champs[2]
            canal = champs[3]
            ...
            try:
                rssi = int(float(rssi_txt))
            except Exception:
                continue

            maintenant = time.time()

            if mac in dernier_temps_wifi:
                intervalle = maintenant - dernier_temps_wifi[mac]
            else:
                intervalle = 0.0

            dernier_temps_wifi[mac] = maintenant

            wifi_last_mac = mac
            wifi_last_rssi = rssi
            wifi_last_intervalle = intervalle
            wifi_last_message = (
                f"Beacon Wi-Fi | SSID={ssid} | "
                f"MAC={mac} | Canal={canal} | "
                f"RSSI={rssi} dBm | "
                f"Intervalle={intervalle:.3f} s"
            )
            wifi_last_seq += 1

    except Exception as e:
        wifi_last_message = f"Erreur Beacons Wi-Fi : {e}"
        wifi_last_rssi = None
...
def demarrer_scan_wifi_beacons():
    global wifi_beacon_running, wifi_beacon_thread

    if not wifi_beacon_running:
        wifi_beacon_running = True
        wifi_beacon_thread = threading.Thread(
            target=scanner_wifi_beacons_thread,
            daemon=True
        )
        wifi_beacon_thread.start()


def arreter_scan_wifi_beacons():
    global wifi_beacon_running
    wifi_beacon_running = False


def lire_beacon_wifi():
    global wifi_consumed_seq

    if wifi_last_seq > wifi_consumed_seq and wifi_last_message is not None and wifi_last_rssi is not None:
        wifi_consumed_seq = wifi_last_seq
        label_bluetooth.config(
            text=f"Beacon Wi-Fi : {wifi_last_mac} | "
                 f"RSSI : {wifi_last_rssi} dBm | "
                 f"Intervalle : {wifi_last_intervalle:.3f} s"
        )
        return wifi_last_message, wifi_last_rssi, wifi_last_intervalle

    return None, None, None


# =========================
# BEACONS BLE AVEC INTERVALLE
# =========================

def scanner_ble_thread():
    global ble_scan_running
    global ble_last_message, ble_last_rssi, ble_last_mac, ble_last_intervalle
    global ble_last_seq
    ...
    try:
        ...
        while ble_scan_running:
            ...
            match = re.search(
                r"Device\s+([0-9A-Fa-f:]{17}).*RSSI:.*\((-?\d+)\)",
                ligne
            )
            ...
            if match:
                ...
                if mac in dernier_temps_ble:
                    intervalle = maintenant - dernier_temps_ble[mac]
                else:
                    intervalle = 0.0

                dernier_temps_ble[mac] = maintenant

                ble_last_mac = mac
                ble_last_rssi = rssi
                ble_last_intervalle = intervalle
                ble_last_message = (
                    f"Beacon BLE | MAC={mac} | "
                    f"RSSI={rssi} dBm | "
                    f"Intervalle={intervalle:.3f} s"
                )
                ble_last_seq += 1
        ...
        ble_last_message = f"Erreur BLE : {e}"
        ble_last_rssi = None
...
def demarrer_scan_ble():
    global ble_scan_running, ble_thread

    if not ble_scan_running:
        ble_scan_running = True
        ble_thread = threading.Thread(target=scanner_ble_thread, daemon=True)
        ble_thread.start()


def arreter_scan_ble():
    global ble_scan_running
    ble_scan_running = False


def lire_beacon_ble():
    global ble_consumed_seq

    if ble_last_seq > ble_consumed_seq and ble_last_message is not None and ble_last_rssi is not None:
        ble_consumed_seq = ble_last_seq
        label_bluetooth.config(
            text=f"Beacon BLE : {ble_last_mac} | "
                 f"RSSI : {ble_last_rssi} dBm | "
                 f"Intervalle : {ble_last_intervalle:.3f} s"
        )
        return ble_last_message, ble_last_rssi, ble_last_intervalle
...
def demarrer():
    ...
    if mode_selectionne == TECH_WIFI_BEACONS:
        demarrer_scan_wifi_beacons()

    if mode_selectionne == TECH_BLE_BEACONS:
        demarrer_scan_ble()
...
def reinitialiser():
    ...
    label_snr.config(text="SNR / Intervalle : ---")
...
def boucle_acquisition():
    ...
    if mode_selectionne == TECH_WIFI:
        try:
            ...
            label_etat.config(text=f"Erreur Wi-Fi UDP : {e}")
        ...
        message, rssi, intervalle = lire_beacon_wifi()

        if message is not None and rssi is not None:
            label_source.config(text="Source : Beacons Wi-Fi avec tshark")
            ajouter_mesure(rssi, message, intervalle)

    elif mode_selectionne == TECH_BLE_BEACONS:
        message, rssi, intervalle = lire_beacon_ble()

        if message is not None and rssi is not None:
            label_source.config(text="Source : Beacons BLE avec bluetoothctl")
            ajouter_mesure(rssi, message, intervalle)
...
def generer_rapport():
    ...
    valeurs_snr_ou_intervalle = []
    for s in snr_values:
        try:
            valeurs_snr_ou_intervalle.append(float(s))
        except Exception:
            pass
    ...
    with open(chemin, "w") as f:
        ...
        if len(valeurs_snr_ou_intervalle) > 0:
            moyenne = np.mean(valeurs_snr_ou_intervalle)
            if mode_selectionne == TECH_BLE_BEACONS:
                f.write(f"Intervalle moyen BLE : {moyenne:.3f} s\n")
            elif mode_selectionne == TECH_WIFI_BEACONS:
                f.write(f"Intervalle moyen Wi-Fi beacon : {moyenne:.3f} s\n")
            else:
                f.write(f"SNR moyen : {moyenne:.2f} dB\n")
...
def mettre_a_jour_interface():
    ...
    label_message.config(text=f"Message : {messages[-1][:70]}")
    ...
    if snr_values[-1] != "":
        if mode_selectionne == TECH_BLE_BEACONS:
            label_snr.config(text=f"Intervalle BLE : {float(snr_values[-1]):.3f} s")
        elif mode_selectionne == TECH_WIFI_BEACONS:
            label_snr.config(text=f"Intervalle Wi-Fi beacon : {float(snr_values[-1]):.3f} s")
        else:
...
def configurer_graphes():
    ...
    if mode_selectionne == TECH_BLE_BEACONS:
        ax_snr.set_title("Intervalle entre beacons BLE")
        ax_snr.set_ylabel("Intervalle (s)")
    elif mode_selectionne == TECH_WIFI_BEACONS:
        ax_snr.set_title("Intervalle entre beacons Wi-Fi")
        ax_snr.set_ylabel("Intervalle (s)")
    else:
        ax_snr.set_title("SNR LoRa / Valeur complémentaire")
        ax_snr.set_ylabel("SNR / Intervalle")
...
def mettre_a_jour_graphes():
    ...
    for i, s in enumerate(snr_values):
        try:
            valeurs.append(float(s))
            temps_valeurs.append(temps[i])
        except Exception:
            pass

    if len(valeurs) > 0:
        if mode_selectionne == TECH_BLE_BEACONS:
            ax_snr.plot(temps_valeurs, valeurs, label="Intervalle BLE")
        elif mode_selectionne == TECH_WIFI_BEACONS:
            ax_snr.plot(temps_valeurs, valeurs, label="Intervalle Wi-Fi beacon")
        else:
            ax_snr.plot(temps_valeurs, valeurs, label="SNR")
        ax_snr.legend()
...
label_snr = ttk.Label(frame_stats, text="SNR / Intervalle : ---")
