...
PNG_COMPLET = DOSSIER_SORTIE / "rssi_et_duree.png"
...
class AnalyseurBLEV5:
    ...
    def _construire_onglets(self, parent: ttk.Frame) -> None:
        ...
        self.onglet_temporel = ttk.Frame(self.notebook)
        self.onglet_distribution = ttk.Frame(self.notebook)
        ...
        self.notebook.add(self.onglet_temporel, text="RSSI et durée")
        self.notebook.add(self.onglet_distribution, text="Histogramme et canaux")
        self.notebook.add(self.onglet_chrono, text="Chronologie")

        self._construire_vue_generale()
        self._construire_table_trames()
        self._construire_graphes_temporels()
        self._construire_graphes_distribution()
    ...
    def _construire_graphes_temporels(self) -> None:
        cadre = ttk.Frame(self.onglet_temporel, padding=8)
        cadre.pack(fill="both", expand=True)

        self.fig_temporel, (self.ax_rssi, self.ax_duree) = plt.subplots(
            2, 1, figsize=(11, 8)
        )
        ...
        self.canvas_temporel.get_tk_widget().pack(fill="both", expand=True)

    def _construire_graphes_distribution(self) -> None:
        cadre = ttk.Frame(self.onglet_distribution, padding=8)
        cadre.pack(fill="both", expand=True)

        self.fig_distribution, (self.ax_hist, self.ax_canaux) = plt.subplots(
            1, 2, figsize=(12, 7)
        )
        ...
        self.canvas_distribution.get_tk_widget().pack(fill="both", expand=True)
    ...
    def _traiter_file(self) -> None:
        ...
        if nouvelles:
            ...
            self._rafraichir_graphes_temporels()
            self._rafraichir_graphes_distribution()
    ...
    def _rafraichir_graphes_temporels(self) -> None:
        self.ax_rssi.clear()
        self.ax_duree.clear()
        ...
        if trames_plage:
            ...
            temps_trace = [t.debut_s - origine for t in trames_trace]
            durees_trace = [t.duree_us for t in trames_trace]

            temps_rssi = []
            rssis = []
            for x, trame in zip(temps_trace, trames_trace):
                if trame.rssi_dbm is not None:
                    temps_rssi.append(x)
            ...
            self.ax_rssi.set_xlim(max(0.0, debut_x - marge), fin_x + marge)
            self.ax_duree.set_xlim(max(0.0, debut_x - marge), fin_x + marge)
    ...
    def _rafraichir_graphes_distribution(self) -> None:
        self.ax_hist.clear()
        ...
        if trames_plage:
            durees = [t.duree_us for t in trames_plage]
            self.ax_hist.hist(
                durees,
                bins=min(30, max(5, len(set(durees)))),
            )

            compte = Counter(
                t.canal for t in trames_plage
                if t.canal in (37, 38, 39)
            )
            canaux = [37, 38, 39]
            valeurs = [compte.get(c, 0) for c in canaux]
    ...
    def sauvegarder_graphes(self) -> None:
        ...
        self.label_etat.config(
            text=(
                f"Graphiques sauvegardés : {PNG_COMPLET} et "
                f"{PNG_DISTRIBUTION}"
            )
        )
