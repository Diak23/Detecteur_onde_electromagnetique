...
class AnalyseurBLEV5:
    ...
    def _rafraichir_graphes(self) -> None:
        ...
        trames = self.trames_filtrees[-MAX_POINTS:]

        if trames:
            origine = trames[0].debut_s
            temps = [t.debut_s - origine for t in trames]
            durees = [t.duree_us for t in trames]

            temps_rssi = []
            rssis = []

            for x, trame in zip(temps, trames):
                if trame.rssi_dbm is not None:
                    temps_rssi.append(x)
            ...
            self.ax_hist.hist(durees, bins=min(20, max(5, len(set(durees)))))

            compte = Counter(
                t.canal for t in trames
    ...
    def _rafraichir_chronologie(self) -> None:
        ...
        trames = self.trames_filtrees[-250:]
        ...
        if trames:
            ...
            for trame in trames:
                ...
                largeur = trame.duree_us / 1_000_000.0

                rectangle = Rectangle(
                    (debut_rel, trame.canal - 0.25),
                    largeur,
                    0.5,
                )
            ...
            self.ax_chrono.set_xlim(0, max(fin_max, 0.001))
