...
def lire_beacon_ble():
    """
    Mesure les beacons BLE avec bluetoothctl.
    On récupère :
    - MAC
    - RSSI
    - temps d'arrivée
    - intervalle depuis le dernier beacon du même appareil
    """
    ...
    try:
        ...
        mac_detectee = None
        rssi_detecte = None
        temps_detection = None

        while time.time() - debut_scan < 5:
            ...
            if match:
                mac_detectee = match.group(1)
                rssi_detecte = int(match.group(2))
                temps_detection = time.time()
                break
        ...
        if mac_detectee in dernier_temps_ble:
	    intervalle = temps_detection - dernier_temps_ble[mac_detectee]
	else:
	    intervalle = 0
	dernier_temps_ble[mac_detectee] = temps_detection
	message = (
   	    f"Beacon BLE | MAC={mac_detectee} | "
    	    f"RSSI={rssi_detecte} dBm | "
   	    f"Intervalle={intervalle:.3f} s"
        )


            return message, rssi_detecte

    except Exception as e:
        label_etat.config(text=f"Erreur Beacons BLE : {e}")
        return None, None
...
def scanner_ble_thread():
    global ble_scan_running, ble_last_message, ble_last_rssi
    ...
    try:
        ...
        while ble_scan_running:
            ...
            if match:
                ...
                ble_last_message = f"Beacon BLE | MAC={mac}"
                ble_last_rssi = rssi
...
def demarrer_scan_ble():
    ...
    if not ble_scan_running:
        ...
        threading.Thread(target=scanner_ble_thread, daemon=True).start()
...
def lire_beacon_ble():
    if ble_last_message is not None and ble_last_rssi is not None:
        return ble_last_message, ble_last_rssi
