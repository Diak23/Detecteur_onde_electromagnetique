...
def lire_beacon_ble():
    try:
        ...
        if len(lignes) > 0:
            appareil = lignes[0].replace("Device ", "")

            # Valeur temporaire.
            # À remplacer plus tard par le vrai RSSI venant de ton sniffer Bluetooth.
            rssi_simule = -65

            message = f"Beacon BLE détecté : {appareil}"
            return message, rssi_simule
...
def boucle_acquisition():
    ...
    if mode_selectionne == TECH_WIFI:
        ...
        if message is not None and rssi is not None:
            label_source.config(text="Source : beacons BLE")
            ajouter_mesure(rssi, message, "")
