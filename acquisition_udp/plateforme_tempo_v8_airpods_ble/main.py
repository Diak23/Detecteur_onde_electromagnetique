...
APP_TITLE = "Plateforme TEMPO V8 — iBeacon, AirPods et appareils BLE"
...
class IBeaconFrame:
    ...
    raw_hex: str
    source_field: str
    manufacturer: str = "Inconnu"
    device_type: str = "Appareil BLE"
    detection_confidence: str = "faible"
    local_name: str = ""
...
def decode_ble_advertisement(values):
    """
    Décodage multi-format.

    - iBeacon : décodage complet UUID/Major/Minor.
    - Apple/AirPods : détection heuristique à partir du nom local et des
      Manufacturer Specific Data Apple (Company ID 0x004C).
    - La mention AirPods reste une détection probable lorsque le nom local
      n'est pas transmis en clair.
    """
    candidates = [value for value in values if value]
    joined_text = " | ".join(candidates)
    joined_hex = "".join(clean_hex(value) for value in candidates)

    # Priorité au décodage iBeacon.
    for candidate in candidates:
        ...
        if decoded is not None:
            decoded.update({
                "manufacturer": "Apple",
                "device_type": "iBeacon",
                "detection_confidence": "élevée",
                "local_name": "",
            })
            return decoded

    text_lower = joined_text.lower()
    airpods_name = "airpods" in text_lower
    beats_name = "beats" in text_lower

    # Représentations possibles du Company ID Apple dans les champs tshark.
    apple_company = (
        "4c00" in joined_hex
        or "004c" in joined_hex
        or "apple" in text_lower
    )

    # Signature fréquemment observée dans des trames de proximité Apple
    # associées à des accessoires audio. C'est une heuristique, pas une
    # authentification du modèle.
    probable_audio_signature = (
        "0719" in joined_hex
        or "1910" in joined_hex
        or "0f05" in joined_hex
    )
    ...
    if airpods_name:
        ...
        device_type = "Casque Beats"
        confidence = "élevée"
    elif apple_company and probable_audio_signature:
        device_type = "AirPods / accessoire audio Apple probable"
        confidence = "moyenne"
    elif apple_company:
        device_type = "Appareil Apple BLE"
        confidence = "faible"
    else:
        return None

    # Les trames Apple propriétaires n'ont généralement pas d'UUID iBeacon.
    # Un identifiant générique sera ensuite regroupé avec l'adresse MAC.
    return {
        "uuid": "",
        "major": 0,
        "minor": 0,
        "tx_power_dbm": 0,
        "raw_hex": joined_hex[:512],
        "manufacturer": "Apple" if apple_company else "Inconnu",
        "device_type": device_type,
        "detection_confidence": confidence,
        "local_name": (
            "AirPods" if airpods_name
            else ("Beats" if beats_name else "")
        ),
    }
...
class NRFCapture:
    ...
    def _read_stdout(self):
        ...
        for line in self.process.stdout:
            ...
            decoded = decode_ble_advertisement(raw_values)

            if decoded is not None:
                selected_field = ",".join(
                    field for field in self.raw_fields
            ...
            frame = IBeaconFrame(
                ...
                manufacturer=decoded.get("manufacturer", "Inconnu"),
                device_type=decoded.get("device_type", "Appareil BLE"),
                detection_confidence=decoded.get(
                    "detection_confidence",
                    "faible",
                ),
                local_name=decoded.get("local_name", ""),
            )
...
class IBeaconApp(tk.Tk):
    ...
    def _build_interface(self):
        ...
        self.ble_devices_tab = ttk.Frame(notebook)
        ...
        notebook.add(self.ble_devices_tab, text="Appareils BLE détectés")
    ...
    def _build_frames_tab(self):
        columns = (
            "time", "device", "manufacturer", "device_type",
            "confidence", "mac", "uuid", "major", "minor",
        ...
        headings = {
            "time": "Temps",
            "device": "Appareil logique",
            "manufacturer": "Fabricant",
            "device_type": "Type détecté",
    ...
    def _build_ble_devices_tab(self):
        note = (
            "La détection des AirPods est certaine lorsque le nom local "
            "AirPods est présent. Lorsqu'elle repose uniquement sur les "
            "données constructeur Apple, elle est affichée comme probable."
        )
        ttk.Label(
            self.ble_devices_tab,
            text=note,
            wraplength=1250,
        ...
        self.ble_device_table = ttk.Treeview(
            self.ble_devices_tab,
            columns=columns,
            show="headings",
        )
        ...
        for column in columns:
            self.ble_device_table.heading(column, text=headings[column])
            self.ble_device_table.column(
                column,
                width=280 if column in ("uuid", "device_type") else 120,
                anchor="center",
            )

        self.ble_device_table.pack(
            fill="both",
            expand=True,
            padx=12,
            pady=10,
    ...
    def receive_frame(self, frame):
        ...
        logical_key = frame.uuid or (
            f"{frame.manufacturer}:{frame.device_type}:{frame.address}"
        )
        frame.device_id = self.resolver.assign(logical_key, frame.address)
    ...
    def refresh_ble_device_table(self):
        if not hasattr(self, "ble_device_table"):
            return

        for item in self.ble_device_table.get_children():
            self.ble_device_table.delete(item)
        ...
        for device in sorted(grouped):
            ...
            type_counts = Counter(frame.device_type for frame in values)
            manufacturer_counts = Counter(
                frame.manufacturer for frame in values
            )
            confidence_order = {"faible": 0, "moyenne": 1, "élevée": 2}
            confidence = max(
                (frame.detection_confidence for frame in values),
                key=lambda value: confidence_order.get(value, 0),
            )

            self.ble_device_table.insert(
                "",
                "end",
                values=(
                    device,
                    ...
                    latest.uuid,
                    len(values),
                    self.fmt(latest.calibrated_rssi_dbm),
                    (
                        ""
                        if latest.length_bytes is None
                        else latest.length_bytes
                    ),
                    latest.pdu_type_name,
    ...
    def export_results(self):
        ...
        ble_device_rows = []
        grouped_ble = defaultdict(list)
        for frame in self.frames:
            grouped_ble[frame.device_id].append(frame)

        for device in sorted(grouped_ble):
            values = grouped_ble[device]
            latest = max(values, key=lambda frame: frame.timestamp)
            ble_device_rows.append({
                "device_id": device,
                "fabricant": Counter(
                    frame.manufacturer for frame in values
                ).most_common(1)[0][0],
                "type_detecte": Counter(
                    frame.device_type for frame in values
                ).most_common(1)[0][0],
                "confiance_detection": latest.detection_confidence,
                "nom_local": latest.local_name,
                "adresse_mac": latest.address,
                "uuid_ibeacon": latest.uuid,
                "nombre_trames": len(values),
                "rssi_recent_dbm": latest.calibrated_rssi_dbm,
                "longueur_recente_octets": latest.length_bytes,
                "type_pdu_recent": latest.pdu_type_name,
                "canal_recent": latest.channel,
                "derniere_detection_epoch": latest.timestamp,
            })

        self.write_rows(
            folder / "appareils_ble_detectes.csv",
            ble_device_rows,
        )
        self.write_rows(
