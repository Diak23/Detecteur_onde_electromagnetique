...
NRF_SNIFFER_INTERFACE = None
NRF_RSSI_FIELD = "nordic_ble.rssi"
NRF_CHANNEL_FIELD = "nordic_ble.channel"
...
def detecter_interface_nrf():
    try:
        ...
        for ligne in sortie.splitlines():
            if "nrf" in ligne.lower() or "bluetooth le" in ligne.lower():
...
def lire_beacon_ble():
    global NRF_SNIFFER_INTERFACE

    try:
        if NRF_SNIFFER_INTERFACE is None:
            NRF_SNIFFER_INTERFACE = detecter_interface_nrf()

        if NRF_SNIFFER_INTERFACE is None:
            label_etat.config(text="Sniffer nRF51822 non détecté avec tshark -D")
            return None, None

        commande = [
            "sudo", "tshark",
            "-i", NRF_SNIFFER_INTERFACE,
            "-Y", "btle.advertising_header.pdu_type",
            "-T", "fields",
            "-e", "frame.time_epoch",
        ...
        if len(champs) >= 4:
            temps_ble = champs[0]
            mac_ble = champs[1] if len(champs) > 1 else "inconnue"
            type_pdu = champs[2] if len(champs) > 2 else "ADV"
            rssi_txt = champs[3] if len(champs) > 3 else ""
            canal = champs[4] if len(champs) > 4 else ""
            ...
            message = (
                f"BLE Beacon | MAC={mac_ble} | "
                f"Type={type_pdu} | Canal={canal}"
            )

            label_bluetooth.config(
                text=f"Beacon BLE : {mac_ble} | Canal : {canal}"
            )

            return message, rssi

    except subprocess.TimeoutExpired:
        label_etat.config(text="Aucun beacon BLE reçu")
        return None, None

    except Exception as e:
        label_etat.config(text=f"Erreur nRF51822 : {e}")
        return None, None
...
combo_tech = ttk.Combobox(
    frame_tech,
    values=[TECH_WIFI, TECH_BLUETOOTH, TECH_LORA],
    state="readonly"
)
