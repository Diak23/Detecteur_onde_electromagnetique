...
DEFAULT_REFERENCE_RSSI_DBM = -56.0
...
class BLECapture:
    ...
    def save_calibration(self, reference: float, measured: float, samples: int) -> None:
        data = {
    ...
    def apply_rssi_calibration(self, raw_rssi_dbm: Optional[int]) -> Optional[float]:
        if raw_rssi_dbm is None:
            return None
        return float(raw_rssi_dbm) + self.rssi_offset_db
    ...
    def _read_loop(self) -> None:
        ...
        for raw_line in self.process.stdout:
            ...
            rssi_raw_dbm = self._safe_int(rssi)
            rssi_dbm = self.apply_rssi_calibration(rssi_raw_dbm)
        ...
        with self.csv_path.open("a", newline="", encoding="utf-8") as file:
            ...
            writer.writerow(
                [
                    ...
                    "" if frame.rssi_dbm is None else f"{frame.rssi_dbm:.2f}",
                    f"{self.rssi_offset_db:.2f}",
...
class IBeaconApp:
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.calibration_target_count = DEFAULT_CALIBRATION_SAMPLES
        self.reference_rssi_var = tk.StringVar(value=str(DEFAULT_REFERENCE_RSSI_DBM))
        self.calibration_count_var = tk.StringVar(value=str(DEFAULT_CALIBRATION_SAMPLES))
    ...
    def _build_ui(self) -> None:
        ...
        calibration_frame = ttk.LabelFrame(self.root, text="Calibration RSSI à 1 mètre", padding=8)
        calibration_frame.pack(fill="x", padx=8, pady=(0, 8))

        ttk.Label(calibration_frame, text="RSSI de référence (dBm) :").pack(side="left", padx=5)
        ttk.Entry(calibration_frame, textvariable=self.reference_rssi_var, width=8).pack(side="left", padx=5)
        ttk.Label(calibration_frame, text="Échantillons :").pack(side="left", padx=5)
        ttk.Entry(calibration_frame, textvariable=self.calibration_count_var, width=8).pack(side="left", padx=5)
        self.calibration_button = ttk.Button(calibration_frame, text="Démarrer calibration", command=self.start_calibration)
        self.calibration_button.pack(side="left", padx=10)
        ttk.Button(calibration_frame, text="Réinitialiser", command=self.reset_calibration).pack(side="left", padx=5)
        self.calibration_status_var = tk.StringVar(value=f"Offset chargé : {self.capture.rssi_offset_db:+.2f} dB")
        ttk.Label(calibration_frame, textvariable=self.calibration_status_var).pack(side="left", padx=15)
    ...
    def stop_capture(self) -> None:
        ...
        self.status_label.configure(
            text=f"Etat : arrete | CSV : {self.capture.csv_path}"
        )
    ...
    def _update_statistics(self) -> None:
        ...
        self.stat_vars["rssi"].set(
            "RSSI : -- dBm"
            if not rssis
            else f"RSSI calibré : {rssis[-1]:.1f} dBm | moy. {sum(rssis)/len(rssis):.1f}"
        )
    ...
    def start_calibration(self) -> None:
        if not self.running:
            messagebox.showwarning("Calibration", "Démarre d'abord l'acquisition BLE.")
            return
        try:
            float(self.reference_rssi_var.get().replace(",", "."))
            count = int(self.calibration_count_var.get())
        except ValueError:
            messagebox.showerror("Calibration", "Paramètres de calibration invalides.")
            return
        if count < CALIBRATION_MIN_SAMPLES:
            messagebox.showerror("Calibration", f"Utilise au moins {CALIBRATION_MIN_SAMPLES} échantillons.")
            return
    ...
    def _update_calibration_progress(self) -> None:
        ...
        self.calibration_status_var.set(f"Calibration : {count}/{self.calibration_target_count}")
        if count >= self.calibration_target_count:
    ...
    def finish_calibration(self) -> None:
        ...
        measured = self._robust_mean(self.calibration_samples)
        self.capture.rssi_offset_db = reference - measured
        self.capture.save_calibration(reference, measured, len(self.calibration_samples))
        self.calibration_status_var.set(
            f"Terminé | brut {measured:.2f} dBm | offset {self.capture.rssi_offset_db:+.2f} dB"
        )
        messagebox.showinfo(
            "Calibration terminée",
            f"Référence : {reference:.2f} dBm\nMoyenne brute : {measured:.2f} dBm\nOffset : {self.capture.rssi_offset_db:+.2f} dB"
        )
    ...
    def reset_calibration(self) -> None:
        ...
        try:
            ...
            pass
        self.calibration_button.configure(state="normal")
        self.calibration_status_var.set("Calibration réinitialisée : 0.00 dB")
